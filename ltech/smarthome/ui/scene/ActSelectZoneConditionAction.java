...
public class ActSelectZoneConditionAction extends BaseNormalActivity<ActSelectConditionDeviceBinding> implements ISelectAction {
    String deviceName;
    long device_id;
    String floorName;
    ...
    protected void initView() {
        ...
        BaseQuickAdapter<GoItem, BaseViewHolder> baseQuickAdapter = new BaseQuickAdapter<GoItem, BaseViewHolder>(R.layout.item_select, arrayList) { // from class: com.ltech.smarthome.ui.scene.ActSelectZoneConditionAction.1
            /* JADX INFO: Access modifiers changed from: protected */
            @Override // com.chad.library.adapter.base.BaseQuickAdapter
            public void convert(BaseViewHolder helper, GoItem item) {
                helper.setText(R.id.tv_name, item.getMainText()).setText(R.id.tv_sub, item.getSubText()).setGone(R.id.tv_sub, !StringUtils.isTrimEmpty(item.getSubText())).setBackgroundRes(R.id.iv_select, helper.getAdapterPosition() == ActSelectZoneConditionAction.this.selectPosition ? R.mipmap.ic_tick_sel : R.color.transparent);
                ((AppCompatTextView) helper.getView(R.id.tv_name)).getPaint().setFakeBoldText(true);
            }
        };
        this.mAdapter = baseQuickAdapter;
        baseQuickAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() { // from class: com.ltech.smarthome.ui.scene.ActSelectZoneConditionAction$$ExternalSyntheticLambda0
            @Override // com.chad.library.adapter.base.BaseQuickAdapter.OnItemClickListener
            public final void onItemClick(BaseQuickAdapter baseQuickAdapter2, View view, int i) {
                ActSelectZoneConditionAction.this.lambda$initView$0(baseQuickAdapter2, view, i);
            }
        });
    ...
    private void setItemData(List<GoItem> mItemAction) {
        ...
        for (int i = 0; i < this.switch_sum * 2; i++) {
            mItemAction.add(new GoItem().setMainText(i % 2 == 0 ? String.format("%s%s%s", getString(R.string.zone), Integer.valueOf((i / 2) + 1), getString(R.string.on)) : String.format("%s%s%s", getString(R.string.zone), Integer.valueOf((i / 2) + 1), getString(R.string.off))).setSubText(""));
        }
    }
