...
public class ActSelectDeviceGroupForAction extends BaseRoomDeviceGroupActivity<ActSelect3Binding, FtDeviceGroupVM> implements ISelectAction {
    ...
    protected void initView() {
        ...
        this.okTv.setOnClickListener(new View.OnClickListener() { // from class: com.ltech.smarthome.ui.scene.ActSelectDeviceGroupForAction.1
            @Override // android.view.View.OnClickListener
            public void onClick(View v) {
                ArrayList arrayList = new ArrayList();
                for (int i = 0; i < ActSelectDeviceGroupForAction.this.selectRoleIds.size(); i++) {
                    Role roleByRoleId = ((FtDeviceGroupVM) ActSelectDeviceGroupForAction.this.mViewModel).getRoleByRoleId(ActSelectDeviceGroupForAction.this.selectRoleIds.get(i).longValue());
                ...
                ActSelectDeviceGroupForAction actSelectDeviceGroupForAction = ActSelectDeviceGroupForAction.this;
                actSelectDeviceGroupForAction.lambda$initActionView$3(actSelectDeviceGroupForAction);
            }
        });
    }
    ...
    protected boolean filterGroup(Group group) {
        if (this.isExistGateway == null) {
            this.isExistGateway = Boolean.valueOf(Injection.repo().device().getSuperPanel() != null);
        }
        if (this.isExistGateway.booleanValue()) {
    ...
    protected boolean filterDevice(Device device) {
        ...
        if (!this.isExistGateway.booleanValue()) {
            ...
            if (device.isSubDevice() && device.getParam() != null && (baseIrParam = (BaseIrParam) device.getParam(BaseIrParam.class)) != null && baseIrParam.getUnicastAddress() != 0) {
                return false;
            }
        }
            ...
            case 27:
                if (RelaySeparationHelper.isRelaySeparationSub(device)) {
                    if (device.getSubhide() != 1) {
                        ...
                        if (deviceByDeviceId == null || deviceByDeviceId.getParam() == null || deviceByDeviceId.getParam(BleParam.class) == null || ((BleParam) deviceByDeviceId.getParam(BleParam.class)).getGroupId() == 0) {
                        }
                    }
                }
                break;
