...
public class SceneItemProvider extends BaseDeviceProvider<Device> {
    ...
    public SceneItemProvider(FragmentActivity activity, RecyclerView recyclerView, BaseViewModel viewModel, BaseDeviceProvider.OnDataChangeListener onDataChangeListener) {
    ...
    public void onItemClick(BaseViewHolder helper, Device data, int position) {
        this.viewModel.navigation(NavUtils.destination(ActTrig.class).withLong(Constants.PLACE_ID, data.getPlaceId()).withLong(Constants.CONTROL_ID, data.getId()).withInt(Constants.LIGHT_TYPE, ProductRepository.getLightColorType((Object) data)));
    }
}