...
public class CentralAirItemBinding extends BaseDeviceItemBinding {
    ...
    public void bindItem(final BaseViewModel viewModel, ItemBinding itemBinding, int position, final Device item) {
        itemBinding.set(23, R.layout.item_device_central_air_gate).bindExtra(37, Integer.valueOf(ProductRepository.getProductIcon(item))).bindExtra(33, Boolean.valueOf(item.getParam(BleParam.class) != null)).bindExtra(10, new BindingCommand(new BindingConsumer() { // from class: com.ltech.smarthome.ui.control.binding.CentralAirItemBinding$$ExternalSyntheticLambda0
            @Override // com.ltech.smarthome.binding.command.BindingConsumer
            public final void call(Object obj) {
                CentralAirItemBinding.this.lambda$bindItem$0(item, viewModel, (View) obj);
            }
        }));
    }
    ...
    public /* synthetic */ void lambda$bindItem$0(Device device, BaseViewModel baseViewModel, View view) {
        ...
        if (id == R.id.iv_device_more || id == R.id.layout_item_bg) {
            baseViewModel.navigation(NavUtils.destination(ActCentralAirGateway.class).withLong(Constants.PLACE_ID, device.getPlaceId()).withLong(Constants.CONTROL_ID, device.getId()));
        } else {
