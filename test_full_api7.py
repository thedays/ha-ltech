...
def test_api(account, password):
    ...
    if place_list:
        ...
        print(f"✅ Place: {first_place.get('placename')} (placeid={first_place.get('placeid')})")
    ...
    if isinstance(place_list, dict) and 'rows' in place_list and place_list['rows']:
        ...
        if device_list:
            print(f"✅ Device List (total={device_list.get('total', 0)} devices):")
            for device in device_list.get('rows', []):
                print(f"  - {device.get('devicename')}")
                print(f"    * deviceid: {device.get('deviceid')}")
                print(f"    * deviceType: {device.get('deviceType')}")
                print(f"    * productId: {device.get('productId')}")
                print(f"    * status: {device.get('status')}")
                print(f"    * roomid: {device.get('roomid')}")
                print(f"    * FULL DEVICE: {json.dumps(device, ensure_ascii=False)}")

if __name__ == "__main__":
    if len(sys.argv) >= 3:
        test_api(sys.argv[1], sys.argv[2])
    else:
        print("Usage: python test_full_api7.py <account> <password>")