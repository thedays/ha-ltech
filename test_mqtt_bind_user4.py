...
class LtechApiClient:
    ...
    def generate_mqtt_password(self, timestamp=None):
        ...
        return base64.b64encode(password).decode('utf-8'), timestamp
...
def test_api(account, password):
    ...
    if bind_result is not None:
        ...
        password_mqtt, timestamp = client.generate_mqtt_password()
        
        print(f"\n{'='*60}")
        print(f"阿里云IoT MQTT连接信息:")
        print(f"{'='*60}")
        print(f"Broker: {MQTT_BROKER_CN}")
        print(f"Port: 8883 (SSL)")
        print(f"Client ID: {client.product_key}@@@{client.device_name}")
        print(f"Username: {client.device_name}&{client.product_key}")
        print(f"Password: {password_mqtt}")
        print(f"Timestamp: {timestamp}")
        
        print(f"\n{'='*60}")
        print(f"测试MQTT连接 (SSL)...")
        print(f"{'='*60}")
        ...
        try:
            ...
            mqtt_client.username_pw_set(username, password_mqtt)
...
if __name__ == "__main__":
    if len(sys.argv) >= 3:
        ...
        print("Usage: python test_mqtt_bind_user4.py <account> <password>")