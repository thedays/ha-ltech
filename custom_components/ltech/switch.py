...
class LtechSwitch(LtechEntity, SwitchEntity):
    ...
    def _get_device_state(self):
        ...
        if realtime_state:
            # Check for raw status byte from reportinstruct (multi-switch per-zone parsing)
            if "reportinstruct_status_byte" in realtime_state:
                status_byte = realtime_state["reportinstruct_status_byte"]
    ...
    async def async_turn_on(self, **kwargs):
        try:
            ...
            if self.coordinator.mesh_enabled and self.coordinator.mesh_manager:
                if self._zone_index is not None and self._zone_count is not None and self._zone_count > 1:
                    # Multi-zone: use Vendor Model OnOff with zone-specific bitmask
                    mesh_success = await self.coordinator.control_device_via_mesh(
                        self.device_id, "on", True, zone=self._zone_index
                    )
                else:
            ...
            self._is_on = True
            self._update_zone_state(True)
            self.coordinator.protect_device_state(self.device_id, duration=15.0)
        ...
        try:
            ...
            if self.coordinator.mesh_enabled and self.coordinator.mesh_manager:
                if self._zone_index is not None and self._zone_count is not None and self._zone_count > 1:
                    # Multi-zone: use Vendor Model OnOff with zone-specific bitmask
                    mesh_success = await self.coordinator.control_device_via_mesh(
                        self.device_id, "on", False, zone=self._zone_index
                    )
                else:
    ...
    def _update_zone_state(self, on):
        """Update device_states for this switch after a local control command.

        For multi-zone switches, all zones share the same device_id, so we must
        update only this zone's bit in reportinstruct_status_byte instead of
        overwriting the entire state with a blanket is_on (which would make all
        other zones appear to change state too).
        """
        if self._zone_index is not None and self._zone_count is not None and self._zone_count > 1:
            current_state = self.coordinator.device_states.get(self.device_id, {})
            status_byte = current_state.get("reportinstruct_status_byte", 0)
            bit = 1 << (self._zone_index - 1)
            if on:
                status_byte |= bit
            else:
