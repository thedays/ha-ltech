...
class LtechLight(LtechEntity, LightEntity):
    ...
    def is_on(self):
        ...
        # Fallback: parse reportinstruct_status_byte (bit 0 = zone 1).
        # sync overwrites device_state with {"reportinstruct_status_byte": N}
        # after the 15s protection window, so we must recognize it here.
        status_byte = device_state.get("reportinstruct_status_byte")
        if status_byte is not None:
            result = (status_byte & 0x01) != 0
            _LOGGER.debug(f"[LIGHT_STATE] Using reportinstruct_status_byte=0x{status_byte:02X}, is_on={result}")
            return result
    ...
    async def async_turn_on(self, **kwargs):
        ...
        try:
            ...
            # Protect locally-set state from being overwritten by sync for 15s
            self.coordinator.protect_device_state(self.device_id, duration=15.0)
            self.schedule_update_ha_state()

            _LOGGER.info(f"[LIGHT_CONTROL] Turn ON completed for {self.device_id} (state protected 15s)")
    ...
    async def async_turn_off(self, **kwargs):
        try:
            ...
            _LOGGER.info(f"[LIGHT_CONTROL] Turn OFF completed for {self.device_id} (state protected 15s)")
