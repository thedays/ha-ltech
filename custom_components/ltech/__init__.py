...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    _LOGGER.info(f"[SETUP] Starting Ltech integration setup v2.5.0")
    _LOGGER.info(f"[SETUP] Entry data keys: {list(entry.data.keys())}")
    
    import json
    _LOGGER.info(f"[SETUP] Full entry config: {json.dumps({k: str(v)[:50] for k, v in entry.data.items()}, ensure_ascii=False)}")
    ...
    _LOGGER.info(f"[SETUP] Places type: {type(coordinator.places).__name__}")
    _LOGGER.info(f"[SETUP] api.place_id: {api.place_id}")
    
    # 直接使用 api.place_id（在 _async_update_data 中已设置），避免重复提取
    place_id = api.place_id
    if place_id:
        ...
        # 如果 api.place_id 为空，尝试从 coordinator.places 提取
        _LOGGER.warning(f"[SETUP] api.place_id is None, trying to extract from coordinator.places")
        places_list = []
        if isinstance(coordinator.places, dict):
            places_data = coordinator.places.get("data", coordinator.places)
            _LOGGER.info(f"[SETUP] places_data type={type(places_data).__name__}, keys={list(places_data.keys()) if isinstance(places_data, dict) else 'N/A'}")
        ...
        if places_list:
            ...
            _LOGGER.info(f"[SETUP] Starting mesh for place_id={place_id} (from places_list)")
            mesh_task = hass.async_create_task(coordinator.start_mesh(place_id))
            
            def _mesh_done_callback2(task):
                try:
                    result = task.result()
                    _LOGGER.info(f"[SETUP] Mesh setup completed: mesh_enabled={result}")
                except Exception as e:
            ...
            mesh_task.add_done_callback(_mesh_done_callback2)
        else:
            _LOGGER.error(f"[SETUP] No places found, Mesh will NOT start!")
            _LOGGER.error(f"[SETUP] coordinator.places value: {str(coordinator.places)[:500]}")
